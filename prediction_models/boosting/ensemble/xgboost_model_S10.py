...
import pandas as pd
from prediction_models.config import STOCK_SPLIT_AGG, random_state
import mlflow
import numpy as np
...
def train_xgboost_model(process_input, country):
    params, forecast_range, name = process_input
    mlflow.xgboost.autolog(disable=True)
    with mlflow.start_run(run_name=name):
        X = pd.read_csv(f"{STOCK_SPLIT_AGG}\\{country}\\X_train_S10.csv")
        y = pd.read_csv(f"{STOCK_SPLIT_AGG}\\{country}\\y_train_{forecast_range}_S10.csv", index_col="Date", parse_dates=['Date'])
        X_val = pd.read_csv(f"{STOCK_SPLIT_AGG}\\{country}\\X_val_{forecast_range}_S10.csv")
        y_val = pd.read_csv(f"{STOCK_SPLIT_AGG}\\{country}\\y_val_{forecast_range}_S10.csv", index_col="Date", parse_dates=['Date'])
        X.replace([np.inf, -np.inf], [100000, -100000], inplace=True)
        X_val.replace([np.inf, -np.inf], [100000, -100000], inplace=True)
...
if __name__ == "__main__":
    ...
    for _ in range(150):
        params = {"n_estimators": np.random.randint(300, 1001),
    ...
    for i in range(1, 4):
        for j in range(len(params_versions)):
            inputs.append((params_versions[j], i, f"{country} S10 r{i} {j}"))

    train_xgboost_model(inputs[0], country)
