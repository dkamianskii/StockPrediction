...
def train_xgboost_model(process_input):
    ...
    with mlflow.start_run(run_name=name):
        X = pd.read_csv(f"{STOCK_AGG}\\X_train_S1.csv", index_col="Date", parse_dates=['Date'])
        y = pd.read_csv(f"{STOCK_AGG}\\y_train_{forecast_range}_S1.csv", index_col="Date", parse_dates=['Date'])
        X_val = pd.read_csv(f"{STOCK_AGG}\\X_val_{forecast_range}_S1.csv", index_col="Date", parse_dates=['Date'])
        y_val = pd.read_csv(f"{STOCK_AGG}\\y_val_{forecast_range}_S1.csv", index_col="Date", parse_dates=['Date'])
...
if __name__ == "__main__":
    ...
    for i in range(1, 4):
        for j in range(len(params_versions)):
            inputs.append((params_versions[j], i, f"xgboost r{i} {j}"))
